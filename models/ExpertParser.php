...
class ExpertParser extends Model {
    ...
    /**
     * Функция парсинга суточных номеров и номеров п/с из годовых номеров
     * @param type $start
     * @param type $end
     * @return boolean
     */
...
//Открыть полученный документ
            ...
            while (true) {
                ...
                if (@gettype($res->numv) != "NULL") {
                    ...
                    $resOld = ArchiveCallsOld::find()->
                                    where(['ngod' => $ngod])->
                                    andWhere(['>=', 'dprm', $start])->
                                    andWhere(['<=', 'dprm', $end])->limit(1)->one();
                    if (@gettype($resOld->numv) != "NULL") {
                        $expertise->getActiveSheet()->getCell("B" . $i)->setValueExplicit($resOld->numv, 's');
                        $expertise->getActiveSheet()->getCell("C" . $i)->setValueExplicit($resOld->stbr, 's');
                        $expertise->getActiveSheet()->getCell("D" . $i)->setValueExplicit($resOld->dprm, 's');
                    }
                }
                $i++;
            }
