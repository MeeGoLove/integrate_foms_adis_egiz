...
//continue;
            ...
            //Проверка соответствия результата и исхода вызова, проставление 
            //необходимых по умолчанию                        
            switch ($hm->Z_SL->RSLT) {
                //Случаи где НЕЛЬЗЯ ставить Без эффекта, ну раз поставили
                //ставим Улучшение
                case 401: //Оказана помощь, больной оставлен на месте
                case 402: //Доставлен в травмпункт
                case 403: //Доставлен в больницу
                case 404: //Передан специализированной бригаде СМП
                case 415: //Пациент практически здоров
                case 416: //Дежурство
                    if ($hm->Z_SL->ISHOD == 402) {
                        $hm->Z_SL->ISHOD = 401;
                    }
                    break;
                //Смерть это ухудшение, как ни крути
                case 405: //Смерть в присутствии бригады СМП
                case 406: //Смерть в автомобиле СМП
                case 411: //Смерть до приезда бригады СМП
                    if ($hm->Z_SL->ISHOD != 403) {
                        $hm->Z_SL->ISHOD = 403;
                    }
                    break;
                //Случаи где можно ставить Без эффекта
                case 407: //Больной не найден на месте
                case 408: //Отказ от помощи
                case 409: //Адрес не найден
                case 410: //Ложный вызов
                case 412: //Больной увезён до прибытия СМП
                case 413: //Больной обслужен врачом поликлиники до приб. СМП
                case 414: //Вызов отменён
                case 417: //Отказ от транспортировки для госпитализации в стационар
                    if ($hm->Z_SL->ISHOD != 402) {
                        $hm->Z_SL->ISHOD = 402;
                    }
                    break;
            ...
            if ($years < 14 AND ( (int) $lhm->DOCTYPE != 3 OR (int) $lhm->DOCTYPE != 0)) {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->upload()) {
                ...
                if ($model->parseNumbersOfCalls($model->start, $model->end)) {
                    return Yii::$app->response->sendFile('uploads/Готово.xls', 'Готово.xls');
                } else {
